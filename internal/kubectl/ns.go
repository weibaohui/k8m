...
func (k8s *Kubectl) ListNamespace() ([]v1.Namespace, error) {
	list, err := k8s.client.CoreV1().Namespaces().List(context.Background(), metav1.ListOptions{})
...
func (k8s *Kubectl) GetNamespace(name string) (*v1.Namespace, error) {
	Namespace, err := k8s.client.CoreV1().Namespaces().Get(context.Background(), name, metav1.GetOptions{})
	return Namespace, err
}

func (k8s *Kubectl) RemoveNamespace(name string) error {
	err := k8s.client.CoreV1().Namespaces().Delete(context.Background(), name, metav1.DeleteOptions{})
	return err
}
func (k8s *Kubectl) CreateNamespace(ns *v1.Namespace) (*v1.Namespace, error) {
	ns, err := k8s.client.CoreV1().Namespaces().Create(context.Background(), ns, metav1.CreateOptions{})
	return ns, err
}
