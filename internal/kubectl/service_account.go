...
func (k8s *Kubectl) ListServiceAccount(ns string) ([]v1.ServiceAccount, error) {
	list, err := k8s.client.CoreV1().ServiceAccounts(ns).List(context.TODO(), metav1.ListOptions{})
...
func (k8s *Kubectl) GetServiceAccount(ns, name string) (*v1.ServiceAccount, error) {
	ServiceAccount, err := k8s.client.CoreV1().ServiceAccounts(ns).Get(context.TODO(), name, metav1.GetOptions{})
	return ServiceAccount, err
}
func (k8s *Kubectl) RemoveServiceAccount(ns, name string) error {
	err := k8s.client.CoreV1().ServiceAccounts(ns).Delete(context.TODO(), name, metav1.DeleteOptions{})
	return err
}
func (k8s *Kubectl) CreateServiceAccount(sa *v1.ServiceAccount) (*v1.ServiceAccount, error) {
	sa, err := k8s.client.CoreV1().ServiceAccounts(sa.Namespace).Create(context.TODO(), sa, metav1.CreateOptions{})
	return sa, err
}

func (k8s *Kubectl) UpdateServiceAccount(sa *v1.ServiceAccount) (*v1.ServiceAccount, error) {
	sa, err := k8s.client.CoreV1().ServiceAccounts(sa.Namespace).Update(context.TODO(), sa, metav1.UpdateOptions{})
	return sa, err
}
