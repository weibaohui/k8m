...
func (k8s *Kubectl) ListNode() ([]v1.Node, error) {
	list, err := k8s.client.CoreV1().Nodes().List(context.Background(), metav1.ListOptions{})
...
func (k8s *Kubectl) GetNode(name string) (*v1.Node, error) {
	node, err := k8s.client.CoreV1().Nodes().Get(context.Background(), name, metav1.GetOptions{})
	return node, err
}
