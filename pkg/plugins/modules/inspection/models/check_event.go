...
type InspectionCheckEvent struct {
	...
	RecordID    uint      `gorm:"index:idx_record_id" json:"record_id"`        // 关联的巡检执行记录ID
	...
	ScriptName  string    `gorm:"size:255;index:idx_script_name" json:"script_name"` // 检测脚本名称
	Kind        string    `gorm:"size:100" json:"kind"`                        // 检查的资源类型
	CheckDesc   string    `gorm:"type:text" json:"check_desc"`                 // 检查脚本内容描述
	Cluster     string    `gorm:"size:100;index:idx_cluster" json:"cluster"`   // 检查集群
