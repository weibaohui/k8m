...
如果需要为插件系统添加新的能力，请按照以下步骤操作：

1. 在 `pkg/plugins/api` 中定义新的接口
2. 为接口提供 No-Op 实现
3. 使用 `atomic.Value` 和 holder 结构体实现线程安全的存储
4. 提供服务定位器函数（如 `XXXService()`）
5. 提供 `RegisterXXX()` 和 `UnregisterXXX()` 函数
6. 在需要暴露能力的插件中调用 `RegisterXXX()` 注册实现
7. 在需要使用能力的模块中通过 `XXXService()` 获取能力
