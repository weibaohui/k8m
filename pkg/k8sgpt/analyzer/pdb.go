...
func (PdbAnalyzer) Analyze(a common.Analyzer) ([]common.Result, error) {
	...
	var list []*v1.PodDisruptionBudget
	err := kom.Cluster(a.ClusterID).WithContext(a.Context).Resource(&v1.PodDisruptionBudget{}).Namespace(a.Namespace).WithLabelSelector(a.LabelSelector).List(&list).Error
	if err != nil {
		return nil, err
	}
	...
	for key, value := range preAnalysis {
		...
		parent, found := util.GetParent(a.Context, a.ClusterID, value.PodDisruptionBudget.ObjectMeta)
		if found {
