...
func (analyzer CronJobAnalyzer) Analyze(a common.Analyzer) ([]common.Result, error) {
	...
	err := kom.Cluster(a.ClusterID).WithContext(a.Context).Resource(&batchv1.CronJob{}).Namespace(a.Namespace).WithLabelSelector(a.LabelSelector).List(&cronJobList).Error
	if err != nil {
		return nil, err
	}
