...
// AddPVCCount 增加pvc统计数据
func (p *pvcService) AddPVCCount(selectedCluster string, pvc *corev1.PersistentVolumeClaim) {
...
// RemovePVCCount 减少pvc统计数据
func (p *pvcService) RemovePVCCount(selectedCluster string, pvc *corev1.PersistentVolumeClaim) {
...
func (p *pvcService) Watch() {
	...
	if err != nil {
		klog.Errorf("Error add cron job for PVC: %v\n", err)
	}
	inst.Start()
