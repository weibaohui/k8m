...
// CreateFromWebhookLog 从WebhookLog创建数据库记录
func (w *WebhookLogRecord) CreateFromWebhookLog(webhookID uint, webhookLog interface{}) error {
	// 将webhookLog转换为JSON字符串
	detailBytes, err := json.Marshal(webhookLog)
	if err != nil {
		return err
	}

	// 如果传入的是map类型，尝试解析字段
	if logMap, ok := webhookLog.(map[string]interface{}); ok {
		w.Detail = string(detailBytes)
		
		// 解析summary
		if summary, exists := logMap["summary"]; exists {
			if summaryStr, ok := summary.(string); ok {
				w.Summary = summaryStr
			}
		}

		// 解析request信息
		if request, exists := logMap["request"]; exists {
			if reqMap, ok := request.(map[string]interface{}); ok {
				...
				if webhookName, exists := reqMap["webhook_name"]; exists {
					if nameStr, ok := webhookName.(string); ok {
						w.WebhookName = nameStr
					}
				}
				if receiverID, exists := reqMap["receiver_id"]; exists {
					if idStr, ok := receiverID.(string); ok {
						w.ReceiverID = idStr
					}
				}
				if timestamp, exists := reqMap["timestamp"]; exists {
		...
		// 解析response信息
		if response, exists := logMap["response"]; exists {
			if respMap, ok := response.(map[string]interface{}); ok {
				if statusCode, exists := respMap["status_code"]; exists {
					if code, ok := statusCode.(float64); ok {
						w.StatusCode = int(code)
					}
				}
				if success, exists := respMap["success"]; exists {
					if successBool, ok := success.(bool); ok {
						w.Success = successBool
					}
				}
				if duration, exists := respMap["duration"]; exists {
					if durationFloat, ok := duration.(float64); ok {
						w.Duration = int64(durationFloat)
					}
				}
				if errorMsg, exists := respMap["error_message"]; exists {
					if errStr, ok := errorMsg.(string); ok {
						w.ErrorMessage = errStr
					}
				}
			}
		}
	} else {
		// 如果不是map类型，直接存储为detail
		w.Detail = string(detailBytes)
	}

	w.WebhookID = webhookID
	
	// 如果RequestTime为空，使用当前时间
	if w.RequestTime.IsZero() {
