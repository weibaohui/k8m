...
func (c *InspectionRecord) GetRecordBothContentById(recordID uint) (string, string, int, error) {
	record := &InspectionRecord{}
	record, err := record.GetOne(nil, func(db *gorm.DB) *gorm.DB {
		return db.Where("id = ?", recordID)
	})
	if err != nil {
		return "", "", 0, fmt.Errorf("未找到对应的巡检记录: %d", recordID)
	}

	// 返回AI总结和原始结果，允许为空
	return record.AISummary, record.ResultRaw, record.ErrorCount, nil
}
