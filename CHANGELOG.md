...
**v0.0.187**
1. 新增集群Event转发到 webhook，支持按关键字过滤，支持AI总结，支持自定义格式推送webhook。支持多集群，多监控条件。
1. 调整Pod实时用量占比显示逻辑，改为Limit为分母，未设置Limit时，显示为无穷大 
1. 巡检计划增加开关，当巡检结果没有检查失败项时，可跳过发送webhook通知
...
1. 修复mcp服务tools失效问题 
2. 新增k8m多实例自动选举功能，选中的实例启动巡检 
3. 为集群增加心跳检测及重试功能，在多集群管理界面显示
4. 巡检报告在AI汇总失败时返回带标记的基础汇总，可区分是否因为AI总结失败 
